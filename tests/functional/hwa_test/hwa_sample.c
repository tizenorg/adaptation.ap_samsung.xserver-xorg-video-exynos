...
int get_crtc_via_randr( char* output_to_find )
{
	...
	xcb_randr_get_screen_resources_reply_t* screen_res_reply = NULL;
	xcb_randr_crtc_t first_randr_crtc;
	int i = 0, j = 0;
	...
	screen_res_cookie = xcb_randr_get_screen_resources( c, x_window_dummy );

	//Receive reply from X server
	screen_res_reply = xcb_randr_get_screen_resources_reply( c, screen_res_cookie, 0);

	if(screen_res_reply)
	{

	    /* We go through CRTC, and build a screen for each one. */
        xcb_randr_crtc_t *randr_crtcs = xcb_randr_get_screen_resources_crtcs(screen_res_reply);

        for (i = 0; i < screen_res_reply->num_crtcs; i++)
            {
                ...
                xcb_randr_get_crtc_info_reply_t *crtc_info_r =
                        xcb_randr_get_crtc_info_reply (c, crtc_info_c, NULL);
                ...
                for (j = 0; j < xcb_randr_get_crtc_info_outputs_length (crtc_info_r); j++)
                {
                    xcb_randr_get_output_info_cookie_t output_info_c =
                            xcb_randr_get_output_info (c, randr_outputs [j], XCB_CURRENT_TIME);
                    xcb_randr_get_output_info_reply_t  *output_info_r =
                            xcb_randr_get_output_info_reply (c, output_info_c, NULL);
                    ...
                    memcpy (name, xcb_randr_get_output_info_name (output_info_r), len);
                    name [len] = '\0';
                    ...
                    */
                    ...
                    if (!strcmp (name, output_to_find)){
                        first_randr_crtc = output_info_r->crtc;
                        free (output_info_r);
                        free (crtc_info_r);
                        printf ("Randr CRTC ID= %d\n",first_randr_crtc );
                        return first_randr_crtc;
                    }
